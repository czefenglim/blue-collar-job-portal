...
## Highlights
- One‑to‑one: `User ↔ UserProfile`, `User ↔ Company`, `Company ↔ CompanyVerification`, `Job ↔ JobAnalytics`, `JobApplication ↔ Conversation`, `Company ↔ Subscription`.
- One‑to‑many: `Company → Job`, `Industry → Company`, `Industry → Job`, `User → JobApplication/SavedJob/Notification/Message/Review/Report/PasswordResetOTP/ResumeAnswer`, `Subscription → Invoice`, `Conversation → ChatMessage`.
